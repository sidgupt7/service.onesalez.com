...
export function ForgotPasswordDialog({
  ...
  return (
    ...
    >
      <form
        onSubmit={submit}
        className="w-full max-w-md rounded-3xl bg-[var(--surface)] p-5 shadow-2xl sm:p-7"
      >
        <div className="flex items-start justify-between">
          <div>
            ...
            <h2 id="forgot-password-title" className="mt-4 text-xl font-bold">
              Reset password
            </h2>
            <p className="mt-1 text-sm text-[var(--muted)]">
              We will send a time-limited reset link if the account exists.
            </p>
          </div>
          <button
            type="button"
        ...
        {message ? (
          ...
          <>
            <div className="mt-6 grid grid-cols-2 rounded-xl bg-[var(--surface-soft)] p-1">
              <RealmChoice
                active={realm === "client"}
                label="Client"
                onClick={() => setRealm("client")}
              />
              <RealmChoice
                active={realm === "employee"}
                label="Employee"
                onClick={() => setRealm("employee")}
              />
            </div>
...
function RealmChoice({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`h-10 rounded-lg text-sm font-semibold ${active ? "bg-[var(--surface)] text-[var(--brand)] shadow-sm" : "text-[var(--muted)]"}`}
    >
      {label}
    </button>
  );
