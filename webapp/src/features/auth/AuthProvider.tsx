...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const applySession = useCallback(
    (session: authApi.AuthSession) => {
      const identity = `${session.actor.type}:${session.actor.id}`;
      if (sessionActor.current !== identity) queryClient.clear();
      sessionActor.current = identity;
  ...
  const signIn = useCallback(
    async (credentials: LoginCredentials, quickPin?: string) => {
      const session = await authApi.login(credentials);
      if (quickPin) {
        const deviceName = /Windows/i.test(navigator.userAgent)
          ? "Windows device"
          : "Trusted browser";
        await authApi.enrollTrustedDevice(
  ...
  const signInWithPin = useCallback(
    async (profile: TrustedDeviceProfile, pin: string) => {
  ...
  const authenticatedRequest = useCallback(
    ...
    [accessToken, applySession, queryClient],
  );
