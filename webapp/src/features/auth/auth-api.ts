...
export async function enrollTrustedDevice(
  ...
  pin: string,
  deviceName: string,
): Promise<TrustedDeviceProfile> {
  const enrollment = await apiRequest<TrustedDeviceEnrollment>(
    "/auth/trusted-device",
    {
      method: "POST",
...
export function trustedProfiles(): TrustedDeviceProfile[] {
  try {
    const stored = JSON.parse(
      localStorage.getItem(TRUSTED_DEVICES_KEY) || "[]",
    ) as TrustedDeviceProfile[];
    const now = Date.now();
    const active = stored.filter(
      (profile) => Date.parse(profile.expiresAt) > now,
    );
    if (active.length !== stored.length)
      localStorage.setItem(TRUSTED_DEVICES_KEY, JSON.stringify(active));
    return active;
  } catch {
    return [];
  }
...
export function saveTrustedProfile(profile: TrustedDeviceProfile): void {
  const profiles = trustedProfiles().filter(
    (item) =>
      item.deviceId !== profile.deviceId &&
      item.actor.email !== profile.actor.email,
  );
...
export function forgetTrustedProfile(deviceId: string): void {
  localStorage.setItem(
    TRUSTED_DEVICES_KEY,
    JSON.stringify(
      trustedProfiles().filter((profile) => profile.deviceId !== deviceId),
    ),
  );
}
...
export function forgetTrustedProfilesForActor(email: string): void {
  localStorage.setItem(
    TRUSTED_DEVICES_KEY,
    JSON.stringify(
      trustedProfiles().filter(
        (profile) => profile.actor.email.toLowerCase() !== email.toLowerCase(),
      ),
    ),
  );
}
...
export function refreshSession(): Promise<AuthSession> {
  ...
  return navigator.locks
    ? navigator.locks.request("onesalez-session-refresh", refresh)
    : refresh();
}
