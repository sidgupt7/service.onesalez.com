...
describe("LoginPage", () => {
  ...
  it("switches between client and employee workspaces", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /sign in to service console/i }),
    ).toBeInTheDocument();
  });
  ...
  it("shows helpful validation messages", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /sign in to client portal/i }),
    );
  ...
  it("reveals PIN enrollment fields for a trusted device", async () => {
    ...
    await user.click(
      screen.getByRole("checkbox", { name: /enable 6-digit pin/i }),
    );

    expect(screen.getByLabelText("Create PIN")).toHaveAttribute(
      "type",
      "password",
    );
    expect(screen.getByLabelText("Confirm PIN")).toHaveAttribute(
      "type",
      "password",
    );
  });
