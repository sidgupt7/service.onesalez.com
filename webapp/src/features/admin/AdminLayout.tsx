...
const navigation = [
  ...
  { label: "Teams", icon: Users, path: "/admin/teams", enabled: true },
  ...
  { label: "Settings", icon: Settings, path: "/admin/settings", enabled: true },
] as const;
...
export function AdminLayout() {
  ...
  const workspaceLabel = actor?.roles.includes("SYSTEM_ADMIN")
      ...
      >
              ...
              .filter(
                (item) =>
                  (item.label !== "Leads" ||
                    ...
                    actor?.permissions.includes("leads.manage")) &&
                  (!["Employees", "Teams"].includes(item.label) ||
                    actor?.roles.includes("SYSTEM_ADMIN")) &&
                  (item.label !== "Reports" ||
                    actor?.roles.includes("SYSTEM_ADMIN") ||
                    actor?.permissions.includes("analytics.view")),
              )
              .map(({ label, icon: Icon, enabled, ...item }) =>
                enabled ? (
                  <NavLink
                    key={label}
                    to={item.path}
                  ...
                  <div
                    key={label}
                    className="flex h-11 items-center gap-3 rounded-xl px-3 text-sm text-slate-500"
                    aria-disabled="true"
                  >
                    <Icon aria-hidden="true" className="h-[18px] w-[18px]" />
                    <span>{label}</span>
                    <span className="ml-auto rounded-full bg-white/6 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-slate-500">
                      Soon
                    </span>
                  </div>
                ),
        ...
        <div className="border-t border-white/8 p-3">
          <div className="rounded-2xl bg-white/5 p-3">
            <div className="flex items-center gap-3">
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-blue-400/15 text-sm font-bold text-blue-200">
                {initials}
              </div>
        ...
        <header className="sticky top-0 z-30 flex h-18 items-center justify-between border-b border-[var(--border)] bg-[color:var(--surface)]/92 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden h-10 items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-2.5 sm:flex">
              <div className="grid h-7 w-7 place-items-center rounded-lg bg-[var(--brand)] text-[10px] font-bold text-white">
                {initials}
              </div>
