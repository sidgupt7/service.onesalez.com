...
export function ClientOnboardingWizard({
  ...
  const {
    ...
    defaultValues: { locationType: "HEAD_OFFICE", ...initialValues },
  });
  ...
  return (
    <div
      className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/55 p-0 backdrop-blur-sm sm:items-center sm:p-5"
      ref={dialogRef}
      tabIndex={-1}
      role="dialog"
      aria-modal="true"
        ...
        <div className="flex items-start justify-between border-b border-[var(--border)] px-5 py-5 sm:px-7">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted)]">
              Company, first location, and client administrator.
            </p>
          </div>
          <button
            type="button"
            ...
            onClick={onClose}
            disabled={pending}
            className="grid h-10 w-10 place-items-center rounded-xl text-[var(--muted)] hover:bg-[var(--surface-soft)]"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 border-b border-[var(--border)] px-5 py-4 sm:px-7">
          {["Business", "Location", "Administrator"].map((label, index) => (
            <div key={label} className="flex items-center gap-2">
              <span
                className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold ${index <= step ? "bg-[var(--brand)] text-white" : "bg-[var(--surface-soft)] text-[var(--muted)]"}`}
              >
                {index < step ? <Check className="h-4 w-4" /> : index + 1}
              </span>
              <span
                className={`hidden text-xs font-semibold sm:block ${index <= step ? "text-[var(--text)]" : "text-[var(--muted)]"}`}
              >
                {label}
              </span>
            </div>
        ...
        >
          <div className="overflow-y-auto px-5 py-6 sm:px-7">
            {step === 0 && (
              <Section
                ...
                description="The legal client record used throughout the CRM."
              >
                <Field
                  label="Client code"
                  required
                ...
                >
                  <Input
                    {...register("legalName")}
                    placeholder="Name on registration documents"
                  />
                </Field>
                <Field label="Display name" error={errors.displayName?.message}>
                  <Input
                    {...register("displayName")}
                    placeholder="Short trading name (optional)"
                  />
                </Field>
                ...
                >
                  <Input {...register("primaryPhone")} placeholder="Optional" />
                </Field>
              </Section>
            )}
            ...
            {step === 1 && (
              <Section
                ...
                description="This is created as the primary location. More locations can be added later."
              >
                <Field
                  label="Location code"
                  required
                ...
                >
                  <select
                    ...
                    className="h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3.5 text-sm outline-none focus:border-[var(--brand)] focus:ring-4 focus:ring-[var(--focus)]"
                  >
                ...
                <Field label="State" required error={errors.stateName?.message}>
                  <Input {...register("stateName")} />
                </Field>
                <Field
                  label="PIN code"
                  required
            ...
            {step === 2 && (
              <Section
                ...
                description="This person can sign in and manage other contacts for this client."
              >
                <Field
                  label="Full name"
                  required
                ...
                >
                  <Input {...register("adminName")} autoFocus />
                </Field>
                <Field label="Designation" error={errors.designation?.message}>
                  <Input
                    {...register("designation")}
                    placeholder="Owner, IT Manager…"
                  />
                </Field>
                <Field
                ...
                >
                  <Input {...register("mobileNumber")} inputMode="tel" />
                </Field>
                <Field
                ...
                >
                  ...
                  <p className="mt-1.5 text-xs text-[var(--muted)]">
                    At least 12 characters. Share it securely with the client
                    administrator.
                  </p>
                </Field>
              </Section>
          ...
          <div className="flex items-center justify-between border-t border-[var(--border)] px-5 py-4 sm:px-7">
            <Button
              ...
              disabled={pending}
              className="bg-transparent px-3 text-[var(--muted)] shadow-none hover:bg-[var(--surface-soft)] hover:text-[var(--text)]"
            >
