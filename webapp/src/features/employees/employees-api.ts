...
export type EmployeeRole =
  "SERVICE_EMPLOYEE" | "SERVICE_ADMIN" | "SYSTEM_ADMIN";
...
export function listEmployees(
  ...
  return request<EmployeePage>(
    `/users?${new URLSearchParams({ paginated: "1", page: String(page), limit: "25", search })}`,
  );
}
...
export function addEmployee(
  ...
  return request<{ employee_id: number }>("/users", {
    method: "POST",
...
export function updateEmployee(
  ...
  return request<EmployeeSummary>(`/users/${id}`, {
    method: "PUT",
    body: JSON.stringify(input),
  });
}
...
export function suspendEmployee(
  ...
  return request<{ message: string }>(`/users/${id}/suspend`, {
    method: "POST",
...
export function reactivateEmployee(
  ...
  return request<{ message: string }>(`/users/${id}/reactivate`, {
    method: "POST",
  });
}
