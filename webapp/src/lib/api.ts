...
const API_BASE_URL = import.meta.env.PROD ? '/api/v1' : (import.meta.env.VITE_API_BASE_URL || '/api/v1');
...
export async function apiRequest<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new ApiError('Cannot reach the service. Reload this page and try again.', 0, 'NETWORK_ERROR');
  }
  let payload: ApiEnvelope<T>;
  try {
    payload = (await response.json()) as ApiEnvelope<T>;
  } catch {
    throw new ApiError('The service returned an unexpected response. Reload this page and try again.', response.status, 'INVALID_RESPONSE');
  }
