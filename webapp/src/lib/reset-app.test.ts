...
describe("clearAppCache", () => {
  it("clears this app worker and caches while retaining preferences and unrelated caches", async () => {
    ...
    vi.stubGlobal("navigator", {
      serviceWorker: {
        getRegistrations: vi.fn().mockResolvedValue([
          { scope, unregister: unregisterApp },
          { scope: scope + "other/", unregister: unregisterOther },
        ]),
      },
    });
    ...
    vi.stubGlobal("caches", {
      keys: vi
        ...
        .mockResolvedValue([
          ...
          `workbox-precache-v2-${scope}other/`,
          "other-app",
        ]),
      delete: removeCache,
    });
    ...
    expect(removeCache.mock.calls.map(([name]) => name)).toEqual([
      "onesalez-pages-v2",
      `workbox-precache-v2-${scope}`,
    ]);
    expect(localStorage.getItem("onesalez_trusted_devices_v1")).toBe(
      "saved-device",
    );
  });
