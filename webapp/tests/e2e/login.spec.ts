...
test("employee workspace changes the sign-in context", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: /sign in to service console/i }),
  ).toBeVisible();
});
...
test("administrator can sign in, enter a protected route, and logout", async ({
  page,
}) => {
  ...
  await page
    .getByRole("button", { name: "Sign in to service console" })
    .click();
