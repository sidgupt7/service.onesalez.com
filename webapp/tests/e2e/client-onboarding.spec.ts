...
test('system administrator can onboard a client', async ({ page }) => {
  ...
  const session = {
    ...
    actor: { id: 1, type: 'EMPLOYEE', email: 'admin@onesalez.com', displayName: 'Siddharth Gupta', clientId: null, roles: ['SYSTEM_ADMIN'], permissions: ['clients.manage'] },
  };
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    else if (url.pathname.endsWith('/customers/onboard') && request.method() === 'POST') {
      submitted = request.postDataJSON() as Record<string, any>;
      const clientInput = submitted.client as Record<string, string>;
      data = {
        client_id: 17, client_code: clientInput.client_code, legal_name: clientInput.legal_name,
        display_name: clientInput.display_name, primary_email: submitted.administrator.email,
        primary_phone: null, is_active: true, location_count: 1, contact_count: 1,
      };
      clients.push(data as Record<string, unknown>);
    } else if (url.pathname.endsWith('/customers')) data = clients;
    else {
      await route.fulfill({ status: 404, json: { success: false, data: null, error: { code: 'NOT_FOUND', message: 'Not found.' }, timestamp: new Date().toISOString() } });
      return;
    }
    await route.fulfill({ json: { success: true, data, error: null, timestamp: new Date().toISOString() } });
  });

  await page.goto('/admin/clients');
  await expect(page.getByRole('heading', { name: 'Clients', exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Onboard client' }).first().click();

  await page.getByLabel(/Client code/).fill('TEST001');
  await page.getByLabel(/Legal business name/).fill('Test Trading Private Limited');
